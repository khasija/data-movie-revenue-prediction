...
class BasicPreprocessing(TransformerMixin, BaseEstimator):
    ...
    '''
        ...
        ## 4.1 Drop columns
        df.drop(columns = [ 'imdb_id', 'status', 'belongs_to_collection', 'spoken_languages'], inplace = True) # popularity,'tagline', 'overview',
        ...
        df['genres'] = df['genres'].apply(lambda x: most_common_genres if x == {} else x)  # TODO: Should we say that there is no genre instead?
